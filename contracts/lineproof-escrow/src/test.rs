...
fn test_release_changes_status() {
    ...
    let record = EscrowImpl::get_record(env.clone(), user, Symbol::new(&env, "sneaker-drop")).unwrap();
...
fn test_deposit_rejects_non_positive_amount() {
    ...
    let user = Address::generate(&env);
    let asset = Address::generate(&env);
    EscrowImpl::deposit(env.clone(), user, Symbol::new(&env, "sneaker-drop"), 0i128, asset);
}
...
fn test_deposit_rejects_duplicate_for_same_user_and_queue() {
    ...
    EscrowImpl::deposit(env.clone(), user, Symbol::new(&env, "sneaker-drop"), 300i128, asset);
}
...
fn test_release_already_released_panics() {
    ...
    EscrowImpl::deposit(env.clone(), user.clone(), Symbol::new(&env, "sneaker-drop"), 500i128, asset);
    EscrowImpl::release(env.clone(), admin.clone(), user.clone(), Symbol::new(&env, "sneaker-drop"));
    EscrowImpl::release(env.clone(), admin, user, Symbol::new(&env, "sneaker-drop"));
}
