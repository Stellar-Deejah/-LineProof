  ...
  return {
    ...
    SorobanRpc: {
      ...
      Server: vi.fn(() => ({
        ...
        prepareTransaction: vi.fn(async (tx) => {
...
describe("Contract ID Validation Across All Clients", () => {
  it('QueueClient constructor throws SDKError("INVALID_CONTRACT_ID") for invalid contract ID', () => {
    ...
    expect(
      () => new QueueClient(client, { queueContractId: INVALID_CONTRACT_ID }),
    ).toThrow(SDKError);
  });
  ...
  it('EnrollmentClient constructor/methods throw SDKError("INVALID_CONTRACT_ID") for invalid contract ID', async () => {
    ...
    expect(() => new EnrollmentClient(client, INVALID_CONTRACT_ID)).toThrow(
      SDKError,
    );
    const enrollment = new EnrollmentClient(client);
    await expect(
      enrollment.enroll(INVALID_CONTRACT_ID, "identity"),
  ...
  it('EscrowClient constructor/methods throw SDKError("INVALID_CONTRACT_ID") for invalid contract ID', async () => {
    ...
    expect(() => new EscrowClient(client, INVALID_CONTRACT_ID)).toThrow(
      SDKError,
    );
    const escrow = new EscrowClient(client);
    await expect(
      escrow.deposit(INVALID_CONTRACT_ID, 10, "USDC"),
    ).rejects.toThrow(SDKError);
  });
  ...
  it('IdentityClient constructor/methods throw SDKError("INVALID_CONTRACT_ID") for invalid contract ID', async () => {
    ...
    expect(() => new IdentityClient(client, INVALID_CONTRACT_ID)).toThrow(
      SDKError,
    );
    const identity = new IdentityClient(client);
    await expect(
      identity.bindIdentity(INVALID_CONTRACT_ID, "identity"),
    ).rejects.toThrow(SDKError);
  });
});
...
describe("QueueClient", () => {
  it("getPosition rejects a non-positive positionId", async () => {
    ...
    const queue = new QueueClient(client, {
      queueContractId: VALID_CONTRACT_ID,
    });
    ...
    client.sorobanServer.simulateTransaction = vi.fn().mockResolvedValue({
      ...
      result: {
        retval: xdr.ScVal.scvMap([
          ...
          new xdr.ScMapEntry({
            key: xdr.ScVal.scvSymbol("enrolled_at"),
            val: xdr.ScVal.scvU64(xdr.Uint64.fromString("1234567890")),
          }),
          new xdr.ScMapEntry({
            key: xdr.ScVal.scvSymbol("identity"),
            val: xdr.ScVal.scvString("G_TEST_IDENTITY"),
          }),
          new xdr.ScMapEntry({
            key: xdr.ScVal.scvSymbol("status"),
            val: xdr.ScVal.scvSymbol("advanced"),
          }),
        ]),
      },
    } as any);
  ...
  it("advance polls and parses return value", async () => {
    ...
    const { xdr } = await import("@stellar/stellar-sdk");
    // Use a real valid Stellar address for testing
    const adminKey = "GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF";
    ...
    const advanced = await queue.advance(adminKey, 2);
...
describe("EnrollmentClient", () => {
  it("throws SDKError when credentials missing on enroll", async () => {
    ...
    const enrollment = new EnrollmentClient(client);
    await expect(
      enrollment.enroll(VALID_CONTRACT_ID, "identity"),
    ).rejects.toThrow(SDKError);
  });
  ...
  it("isEnrolled parses boolean from simulateTransaction", async () => {
    ...
    const result = await enrollment.isEnrolled(
      VALID_CONTRACT_ID,
      "GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF",
    );
...
describe("EscrowClient", () => {
  it("rejects non-positive deposit amount", async () => {
    ...
    const callerKey =
      "GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF";
    await expect(
      escrow.deposit(VALID_CONTRACT_ID, callerKey, "queue-id", 0n, "USDC"),
...
describe("IdentityClient", () => {
  it("throws TRANSFER_DISABLED on transfer attempt", async () => {
    ...
    const identity = new IdentityClient(client);
    await expect(identity.recordTransferAttempt("from", "to")).rejects.toThrow(
      "TRANSFER_DISABLED",
    );
  });
  ...
  it("isBound parses boolean from simulateTransaction", async () => {
    ...
    const result = await identity.isBound(
      VALID_CONTRACT_ID,
      "GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF",
    );
...
describe("Credentials and Keypair Validation", () => {
  it('throws SDKError("MISSING_CREDENTIALS") when no private key is configured across all transaction methods', async () => {
    ...
    await expect(
      enrollment.cancel(VALID_CONTRACT_ID, "identity"),
    ).rejects.toThrow(SDKError);
    await expect(queue.advance(1)).rejects.toThrow(SDKError);
    await expect(queue.close()).rejects.toThrow(SDKError);
    await expect(
      identity.bindIdentity(VALID_CONTRACT_ID, "identity"),
    ).rejects.toThrow(SDKError);
    await expect(escrow.deposit(VALID_CONTRACT_ID, 10, "USDC")).rejects.toThrow(
      SDKError,
    );
    await expect(escrow.release(VALID_CONTRACT_ID, "identity")).rejects.toThrow(
      SDKError,
    );
    await expect(escrow.refund(VALID_CONTRACT_ID, "identity")).rejects.toThrow(
      SDKError,
    );
    await expect(escrow.expire(VALID_CONTRACT_ID, "identity")).rejects.toThrow(
      SDKError,
    );
  });
});
